...
Howard = 50 #steps in Howard improvement
...
chi = np.zeros((S,S)) 
ones = np.ones([1,S])
hwdmat = np.zeros((S,1)) # define matrix for Howard improvement
...
start = time.time()
print("Vectorized VFI time + Howard:")
...
while np.any(comp) == 1: 
    valueold = value
    chi = M + beta*(np.matmul( valueold, ones ))
    value = np.array([np.max( chi, axis = 0)]).reshape((S,1))
    cont = np.argmax(chi, axis = 0) #max index row
    #Here start howard:
    for ii in range(S):
        hwdmat[ii] = M[cont[ii],ii]
    for i in range(Howard):
        value = hwdmat+beta*value[cont]
...
cons = np.zeros((S,1))
for i in index:
    cons[i] = (ks[j]**(1-theta))*(h**theta)+(1-delta)*ks[i]-kp[i]
